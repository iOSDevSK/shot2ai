...
• Choose the model for each chat (for example Opus or Sonnet in Claude) in the popup, or for one send in the card. The list is read from the chat's own model picker; if the chat cannot switch (a model it no longer lists, or one that needs another plan), nothing is sent and the card says why.
...
- **Web history**: the address and title of a captured page are kept with that capture on the device, to label it and name saved files. The address of the user's last conversation with each of ChatGPT, Claude, Gemini and Perplexity is kept on the device so a closed chat tab opens again there, with the model the user chose for each and the model names read from its picker. None of it is collected or sent anywhere else.
